...
class G5_NARIYA {
	...
	public function write_insert($board, $wr_id, $w, $qstr, $redirect_url) {
		...
		// 게시판 확장
		if(IS_NA_BBS) {
			global $as_type, $as_extend, $as_down, $as_view, $as_star_score, $as_star_cnt, $as_choice, $as_choice_cnt, $as_tag, $as_img;

			$wr = get_write($write_table, $wr_id, true);

			// 필드 확장
			$as_extend = isset($as_extend) ? (int)$as_extend : 0;
			$as_img = isset($as_img) ? (int)$as_img : 0;
			$as_down = isset($as_down) ? (int)$as_down : 0;
			$as_view = isset($as_view) ? (int)$as_view : 0;
			$as_choice = isset($as_choice) ? (int)$as_choice : 0;
			$as_choice_cnt = isset($as_choice_cnt) ? (int)$as_choice_cnt : 0;
			...
			$set_sql = ", as_extend = '".$as_extend."'
						...
						, as_view = '".$as_view."'
						, as_choice = '".$as_choice."'
						, as_choice_cnt = '".$as_choice_cnt."'";

			// 메인글
			if($is_admin) {
				global $config;

				$as_type = isset($as_type) ? (int)$as_type : 0;
			...
			// 별점
			if(isset($as_star_score)) {
				$as_star_score = isset($as_star_score) ? (int)$as_star_score : 0;
				$as_star_cnt = isset($as_star_cnt) ? (int)$as_star_cnt : 0;
				$set_sql .= ", as_star_score = '".$as_star_score."', as_star_cnt = '".$as_star_cnt."'";
			}
			...
			if($as_extend) {
				...
				if($is_save) {
					...
					if($cnt) {
						...
						$set_sql .= ", wr_content = '".na_pack($data)."'";
					}
				}
			} else {
	...
	public function tail_sub(){
		...
		if($member['mb_id']) {
			...
			if($msg) {
				...
				// 메시지
				echo "<script> $(document).ready(function(){ alert('".$msg."'); });</script>";
			}
		}
	} // end function
}
