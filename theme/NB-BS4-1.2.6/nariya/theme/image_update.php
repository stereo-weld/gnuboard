...
if(!isset($_FILES['img_file']['tmp_name']) || !$_FILES['img_file']['tmp_name']) 
	alert("파일을 등록해 주세요.");

$upload_max_filesize = ini_get('upload_max_filesize');

$_FILES['img_file']['error'] = isset($_FILES['img_file']['error']) ? $_FILES['img_file']['error'] : 0;
if($_FILES['img_file']['error'] == 1) {
	alert('파일의 용량이 서버에 설정('.$upload_max_filesize.')된 값보다 크므로 업로드 할 수 없습니다.');
} else if ($_FILES['img_file']['error'] != 0) {
	alert('파일이 정상적으로 업로드 되지 않았습니다.');
}

$_FILES['img_file']['name'] = isset($_FILES['img_file']['name']) ? $_FILES['img_file']['name'] : '';
...
// 페이지 이동
$goto_url = './image_form.php?mode='.urlencode($mode);

if($fid)
	$goto_url .= '&amp;fid='.urlencode($fid);

if($win)
	$goto_url .= '&amp;win='.urlencode($win);

goto_url($goto_url.$spot);