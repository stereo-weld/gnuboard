            ...
            $.get(g5_bbs_url+"/ajax.autosavelist.php", function(data){
                ...
                if ($(data).find("list").find("item").length > 0) {
                    $(data).find("list").find("item").each(function(i) {
                        ...
                        $("#autosave_pop ul")
                            .append('<li class="list-group-item border-left-0 border-right-0 rounded-0 ellipsis"><button type="button" class="autosave_del btn text-muted p-0 float-right"><i class="fa fa-times"></i></button><a href="#none" class="autosave_load"><span class="text-muted">['+datetime+']</span> '+subject+'</a></li>')
                            .find("li:eq("+i+")")
